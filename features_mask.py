...
class BotDataset(Dataset):
    def __init__(self, name, batch_size, user_num):
        ...
        user_features_numeric = torch.load(path + "My_user_feature_ZS.pt")
        user_features_bool = torch.load(path + "My_user_feature_bool.pt")
        user_features_tweet = torch.load(path + "user_features2.pt")
        user_feature_des = torch.load(path + "user_features3.pt")

        # mask
        numeric_mask = torch.FloatTensor(user_features_numeric.size()).uniform_() < feature_ratio
        bool_mask = torch.FloatTensor(user_features_bool.size()).uniform_() < feature_ratio
        tweet_mask = torch.FloatTensor(user_features_tweet.size()).uniform_() < feature_ratio
        des_mask = torch.FloatTensor(user_feature_des.size()).uniform_() < feature_ratio

        self.user_features_numeric = torch.mul(user_features_numeric, numeric_mask)
        self.user_features_bool = torch.mul(user_features_bool, bool_mask)
        self.user_features_tweet = torch.mul(user_features_tweet, tweet_mask)
        self.user_feature_des = torch.mul(user_feature_des, des_mask) 
...
class HANBotDetector(pl.LightningModule):
    def __init__(self, HAN_hid_out, tweet_in_channel,numeric_in_channels, bool_in_channels,\
        ...
        self.HAN_layers.append(RGTLayer(num_edge_type=2,in_size=linear_out_channels,out_size = HAN_hid_out,\
                    layer_num_heads=num_heads[0], semantic_head=semantic_heads[0], dropout=DROPOUT_RATE))

        for l in range(1,len(num_heads)):
            self.HAN_layers.append(RGTLayer(num_edge_type=2, in_size=HAN_hid_out,\
