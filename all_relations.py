...
class BotDataset(Dataset):
    def __init__(self, name, batch_size, user_num):
        ...
        ########### NEED TO CHANGE ###############################
        des_c00_edge = torch.load(path + "des_cluster2_00_edge.pt")
        des_c01_edge = torch.load(path + "des_cluster2_01_edge.pt")
        des_c11_edge = torch.load(path + "des_cluster2_11_edge.pt")
        bool_c00_edge = torch.load(path + "cluster2_00_edge.pt")
        bool_c01_edge = torch.load(path + "cluster2_01_edge.pt")
        bool_c11_edge = torch.load(path + "cluster2_11_edge.pt")
        # v0_edge = torch.load(path + "v0_edge_index.pt")
        # v1_edge = torch.load(path + "v1_edge_index.pt")
        # v2_edge = torch.load(path + "v2_edge_index.pt")
        f0_edge = torch.load(path + "f0_edge_index_28.pt")
        f1_edge = torch.load(path + "f1_edge_index_28.pt")
        f2_edge = torch.load(path + "f2_edge_index_28.pt")
        s0_edge = torch.load(path + "s0_edge_index_28.pt")
        s1_edge = torch.load(path + "s1_edge_index_28.pt")
        s2_edge = torch.load(path + "s2_edge_index_28.pt")
        b_b_edge_index = torch.load(path + 'b_b_edge_index.pt')
        e_b_edge_index = torch.load(path + "e_b_edge_index.pt")
        e_e_edge_index = torch.load(path + "e_e_edge_index.pt")
        p_b_edge_index = torch.load(path + "p_b_edge_index.pt")
        p_e_edge_index = torch.load(path + "p_e_edge_index.pt")
        p_p_edge_index = torch.load(path + "p_p_edge_index.pt")
        p_s_edge_index = torch.load(path + "p_s_edge_index.pt")
        s_b_edge_index = torch.load(path + "s_b_edge_index.pt")
        s_e_edge_index = torch.load(path + 's_e_edge_index.pt')
        s_s_edge_index = torch.load(path + "s_s_edge_index.pt")
        follower_edge = torch.load(path + "follower_edge.pt")
        following_edge = torch.load(path + "following_edge.pt")
        # follower_edge = torch.stack((follower_edge[1], follower_edge[0]))
        # following_edge = torch.stack((following_edge[1], following_edge[0]))

        same_domain = torch.cat((b_b_edge_index, e_e_edge_index, p_p_edge_index, s_s_edge_index), dim = 1)
        dif_domain = torch.cat((e_b_edge_index, p_b_edge_index, p_e_edge_index, p_s_edge_index, s_b_edge_index, s_e_edge_index), dim = 1)
        
        self.edge_index_list = [des_c00_edge, des_c01_edge, des_c11_edge, bool_c00_edge, bool_c01_edge, bool_c11_edge,\
        follower_edge, following_edge, f0_edge, f1_edge, f2_edge, s0_edge, s1_edge, s2_edge, same_domain, dif_domain]
        ##########################################################
        ...
        #print("edge_type{}".format(self.edge_type.size()))
...
class HANBotDetector(pl.LightningModule):
    def __init__(self, HAN_hid_out, tweet_in_channel,numeric_in_channels, bool_in_channels,\
        ...
        # self.HAN1 = HANLayer(num_edge_type=10, in_size=linear_out_channels, out_size=HAN_hid_out, layer_num_heads=1, dropout=DROPOUT_RATE)
        # self.HAN2 = HANLayer(num_edge_type=10, in_size=HAN_hid_out, out_size=HAN_hid_out, layer_num_heads=1, dropout=DROPOUT_RATE)
        
        self.HAN_layers = nn.ModuleList()
        self.HAN_layers.append(RGTLayer(num_edge_type=19,in_size=linear_out_channels,out_size = HAN_hid_out,\
                    layer_num_heads=num_heads[0], semantic_head=semantic_heads[0], dropout=DROPOUT_RATE))

        for l in range(1,len(num_heads)):
            self.HAN_layers.append(RGTLayer(num_edge_type=19, in_size=HAN_hid_out,\
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        # user_features = self.HAN1(user_features, edge_index_list)
        # user_features = self.HAN2(user_features, edge_index_list)
        ...
        # print(pred.size())
        # print(user_features[batch_id].size())
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        #print(np.shape(edge_index_list))
        ...
        #print(user_features_bool.size())
        #print(user_features_numeric.size())
        ...
        print("Your model's F-1 score on validation dataset is {}".format(F1))

        return accuracy

    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(self.parameters(), lr=LEARNING_RATE, weight_decay=WD, amsgrad=False)
        #optimizer = torch.optim.Adagrad(self.parameters(), lr = LEARNING_RATE, weight_decay=WD)
        #optimizer = torch.optim.Adadelta(self.parameters(), lr = LEARNING_RATE, weight_decay=WD)
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        for HAN in self.HAN_layers:
            user_features = HAN(user_features, edge_index_list, beta = True)
...
#     ckpt_file = listdir(ckpt_folder)
#     val_accu = [float(filename[-11:-5]) for filename in ckpt_file]
#     return max(val_accu)
...
if __name__ == "__main__":
    ...
    model = HANBotDetector(HAN_hid_out=128, tweet_in_channel=IN_FEATURES_TWEET, numeric_in_channels=IN_FEATURES_NUMERIC,\
    ...
    trainer = pl.Trainer(gpus=1, num_nodes=1, max_epochs = MAX_EPOCHS, precision=16,log_every_n_steps=1)#callbacks=[checkpoint_callback]
    ...
    # best_model1 = HANBotDetector.load_from_checkpoint(checkpoint_path=best_path1,HAN_hid_out=128, tweet_in_channel=IN_FEATURES_TWEET, numeric_in_channels=IN_FEATURES_NUMERIC,\
    #     bool_in_channels=IN_FEATURES_BOOL, linear_out_channels=128, dropout_rate=DROPOUT_RATE,user_num=USER_NUM, \
    #     batch_size=BATCH_SIZE, num_heads=NUM_HEAD, des_in_channels=IN_FEATURE_DES, semantic_heads=SEMANTIC_HEAD)

    # best_model2 = HANBotDetector.load_from_checkpoint(checkpoint_path=best_path2,HAN_hid_out=128, tweet_in_channel=IN_FEATURES_TWEET, numeric_in_channels=IN_FEATURES_NUMERIC,\
    #     bool_in_channels=IN_FEATURES_BOOL, linear_out_channels=128, dropout_rate=DROPOUT_RATE,user_num=USER_NUM, \
    #     batch_size=BATCH_SIZE, num_heads=NUM_HEAD, des_in_channels=IN_FEATURE_DES, semantic_heads=SEMANTIC_HEAD)

    # best_model3 = HANBotDetector.load_from_checkpoint(checkpoint_path=best_path3,HAN_hid_out=128, tweet_in_channel=IN_FEATURES_TWEET, numeric_in_channels=IN_FEATURES_NUMERIC,\
    #     bool_in_channels=IN_FEATURES_BOOL, linear_out_channels=128, dropout_rate=DROPOUT_RATE,user_num=USER_NUM, \
    #     batch_size=BATCH_SIZE, num_heads=NUM_HEAD, des_in_channels=IN_FEATURE_DES, semantic_heads=SEMANTIC_HEAD)
    
    #print('best_model1')
    trainer.test(model, test_dataloaders = val_loader, verbose = True)
